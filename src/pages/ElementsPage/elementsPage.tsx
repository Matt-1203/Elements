...
function initImageSequence(config: SequenceConfig): Promise<void> {
  return new Promise((resolve) => {
    ...
    Promise.all(imagePromises).then(() => {
      ...
      const tl = gsap.timeline({scrollTrigger: config.scrollTrigger});
      // Animate the playhead from 0 to the last frame of the sequence
      tl.to(playhead, {frame: images.length - 1, ease: "none", onUpdate: render}, 0);
      // If a callback for timeline creation is provided, call it with the created timeline
      if (config.onTimelineCreated) {config.onTimelineCreated(tl);}
...
function ElementsPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {const heroUrls = Array.from({ length: 250 }, (_, index) => `/sequenceOne/${String(index + 1).padStart(4, "0")}.webp`);
      ...
      const p1 = heroCanvasRef.current && heroSectionRef.current
        ? (heroCanvasRef.current.width = 1920,
           ...
           initImageSequence({urls: heroUrls, canvas: heroCanvasRef.current,
             scrollTrigger: {trigger: heroSectionRef.current, start: "top top", end: "+=3000", scrub: true, pin: true, anticipatePin: 1, refreshPriority: 2},
      ...
      Promise.all([p1]).then(() => {
        ...
        setTimeout(() => {ScrollTrigger.sort(); ScrollTrigger.refresh();}, 100);
      });
    });
    return () => ctx.revert();
  }, []);
  ...
  return (
    <>
      ...
      {isLoading && (
        <Box sx={{position: "fixed", inset: 0, zIndex: 9999, backgroundColor: "#000", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#fff", gap: 2}}>
      ...
      <Box component="main" sx={{opacity: isLoading ? 0 : 1, transition: "opacity 0.5s ease-in-out", position: "relative", color: "#fff", minHeight: "100vh", backgroundColor: "#181818"}}>
        ...
        {/* SECTION 2 */}
        <Box sx={{position: "relative", zIndex: 2, py: 12, px: 3, backgroundColor: "#0a0a0a"}}>
          <Container maxWidth="lg">
            <Typography variant="h4" sx={{fontFamily: "Raleway, sans-serif", fontWeight: 800, textAlign: "center", color: "#ffffff", letterSpacing: -0.5, mb: 8}}>
              PHOTOGRAPHY CONSISTS OF COMPNENTS.
            </Typography>

            <Card sx={{backgroundColor: "#181818", color: "#fff", border: "1px solid #333"}}>
              <CardContent sx={{ p: 5, textAlign: "center" }}>
                <Typography variant="body1" sx={{ color: "#aaa", lineHeight: 1.8, maxWidth: "800px", mx: "auto" }}>
        ...
        <Box sx={{position: "relative", zIndex: 2, py: 12, backgroundColor: "#0a0a0a"}}>
          <Box sx={{ px: { xs: 2, md: 6 } }}>
            <Typography variant="h6" sx={{fontFamily: "Raleway, sans-serif", fontWeight: 700, mb: 3, ml: 1, color: "#888", textTransform: "uppercase", letterSpacing: 1.5,}}>
              VIEW THE ELEMENTS CATALOGUES
            </Typography>

            <Box sx={{display: "flex", gap: 3, overflowX: "auto", pb: 3, pt: 1, "&::-webkit-scrollbar": { height: "8px" }, "&::-webkit-scrollbar-track": { backgroundColor: "#111" }, "&::-webkit-scrollbar-thumb": { backgroundColor: "#333", borderRadius: "4px" }, "&::-webkit-scrollbar-thumb:hover": { backgroundColor: "#555" }}}>
              {GALLERY_CARDS.map((card, idx) => (
                <Card key={idx} sx={{minWidth: { xs: 280, sm: 320, md: 360 }, maxWidth: { xs: 280, sm: 320, md: 360 }, flexShrink: 0, backgroundColor: "#181818", color: "#fff", border: "1px solid #333", display: "flex", flexDirection: "column", justify: "space-between"}}>
                  ...
                  <CardActions sx={{ px: 3, pb: 3, pt: 0 }}>
                    {card.link && card.link !== "" && (
                      <Button component={Link} href={typeof card.link === "string" ? card.link : (card.link as any).ElementsOne} target="_blank" rel="noopener noreferrer" size="small" sx={{ color: "#fff", fontWeight: 700, p: 0, "&:hover": { color: "#aaa" } }}>
                        View More →
                      </Button>
                    )}
                    {card.link === "" && (
